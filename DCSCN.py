...
class SuperResolution(tf_graph.TensorflowGraph):
    ...
    def load_graph(self, frozen_graph_filename='./model_to_freeze/frozen_model_optimized.pb'):
        ...
        if self.dropout_rate < 1:
            self.dropout = self.get_tensor_by_name(
                "prefix/dropout_keep_rate:0")
    ...
    def build_optimizer(self):
        ...
        self.lr_input = tf.placeholder(
            tf.float32, shape=[], name="LearningRate")

        diff = tf.subtract(self.y_, self.y, "diff")

        if self.use_l1_loss:
            self.mse = tf.reduce_mean(
                tf.square(diff, name="diff_square"), name="mse")
        ...
        if self.l2_decay > 0:
            ...
            if self.enable_log:
                tf.summary.scalar("L2WeightDecayLoss/" +
                                  self.name, l2_norm_loss)
    ...
    def add_optimizer_op(self, loss, lr_input):
        ...
        if self.optimizer == "gd":
            ...
            optimizer = tf.train.RMSPropOptimizer(
                lr_input, momentum=self.momentum)
        else:
            print(
    ...
    def log_to_tensorboard(self, test_filename, psnr, save_meta_data=True):
        ...
        org_image = util.set_image_alignment(util.load_image(
            test_filename, print_console=False), self.scale)
        ...
        util.log_scalar_value(self.train_writer, 'LR',
                              self.lr, self.epochs_completed)
        self.train_writer.flush()

        util.log_scalar_value(self.test_writer, 'PSNR',
                              psnr, self.epochs_completed)
    ...
    def do_for_file(self, file_path, output_folder="output"):
        ...
        util.save_image(output_folder + filename +
                        "_bicubic" + extension, scaled_image)
        ...
        if len(org_image.shape) >= 3 and org_image.shape[2] == 3 and self.channels == 1:
            ...
            util.save_image(output_folder + filename +
                            "_bicubic_y" + extension, scaled_image)
            output_y_image = self.do(input_y_image)
            util.save_image(output_folder + filename +
                            "_result_y" + extension, output_y_image)
    ...
    def do_for_evaluate_with_output(self, file_path, output_directory, print_console=False):
        ...
        util.save_image(output_directory + filename +
                        "_input_bicubic" + extension, input_bicubic_image)
        ...
        if true_image.shape[2] == 3 and self.channels == 1:
            ...
            util.save_image(output_directory + filename +
                            "_input" + extension, input_y_image)
            util.save_image(output_directory + filename +
                            "_input_bicubic_y" + extension, input_bicubic_y_image)
            util.save_image(output_directory + filename +
                            "_true_y" + extension, true_ycbcr_image[:, :, 0:1])
            util.save_image(output_directory + filename +
                            "_result" + extension, output_y_image)
            util.save_image(output_directory + filename +
                            "_result_c" + extension, output_color_image)
            util.save_image(output_directory + filename +
                            "_loss" + extension, loss_image)
            ...
            util.save_image(output_directory + file_path, true_image)
            util.save_image(output_directory + filename +
                            "_result" + extension, output_image)
        else:
            return None, None
    ...
    def print_steps_completed(self, output_to_logging=False):
        ...
        status = "Finished at Total Epoch:%d Steps:%s Time:%02d:%02d:%02d (%2.3fsec/step) %d x %d x %d patches" % (
            self.epochs_completed, "{:,}".format(
                self.step), h, m, s, processing_time,
